<template>
	<div class="screen_body" :class="{flag:!flag}">
		<div
			class="screen"
			ref="screen"
...
export default {
	...
	computed: {
		translateX() {
			...
			let Footer = document.querySelector(".screen_body")
				console.log(Footer);
				// let offsetX = W > H ? clientHeight : Footer.clientHeight
			return Footer.clientHeight * this.index
		},
		translate() {
	...
	methods: {
		orientationchange() {
			if (window.orientation === 180 || window.orientation === 0) {
                this.index = 0;
                this.$refs.screen && (this.$refs.screen.style.transform = `translateY(0px)`)
				this.flag = true
			}
		...
		touchmove(event) {
			// this.disable && event.preventDefault()
